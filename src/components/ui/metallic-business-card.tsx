...
type Metal = 'gold' | 'silver' | 'bronze' | 'platinum' | 'emerald' | 'black';
...
> = {
  ...
  emerald: {
    ink: 'oklch(0.95 0.02 160)',      // near-white with slight green tint
    sub: 'oklch(0.80 0.03 160)',      // light sage
    glow1: 'rgba(180, 240, 200, .45)',
    glow2: 'rgba(200, 255, 220, .22)',
  },
  black: {
    ink: 'oklch(0.92 0 0)',           // bright white ink on dark
    sub: 'oklch(0.70 0 0)',           // muted grey
    glow1: 'rgba(255, 255, 255, .35)',
    glow2: 'rgba(200, 200, 210, .18)',
  },
};
...
export default function MetallicBusinessCard({
  ...
  const titleId = `card-title-${reactId.replace(/:/g, '')}`;

  const ink = innerMetal ? METAL_TOKENS[innerMetal] : METAL_TOKENS[metal];

  const innerPanelBg = innerMetal
    ? `linear-gradient(180deg, rgba(255,255,255,0.15), rgba(255,255,255,0.04)),
       conic-gradient(from var(--gradient-rotation) at 50% 50%, rgba(0,0,0,0.2) -120deg, rgba(255,255,255,0.3) 9deg, rgba(0,0,0,0.12) 60deg, rgba(0,0,0,0.02) 107deg, rgba(0,0,0,0.1) 131deg, transparent 188deg, rgba(0,0,0,0.2) 240deg, rgba(255,255,255,0.3) 367deg),
       var(--bg-inner)`
    : `linear-gradient(180deg, rgba(255,255,255,0.08), rgba(255,255,255,0.02)),
       radial-gradient(80% 40% at 50% 0%, rgba(255,255,255,0.06), transparent 60%)`;

  const innerPanelShadow = innerMetal
    ? `inset 0 0 0 1px rgba(255, 204, 112, 0.35), 0 1px 4px rgba(0,0,0,0.4)`
      ...
      style={
        {
          ...
          '--glow-1': ink.glow1,
          '--glow-2': ink.glow2,
          ...(innerMetal && { '--bg-inner': METAL_BG[innerMetal] }),
